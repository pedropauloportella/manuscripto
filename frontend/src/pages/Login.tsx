    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        ...
        <div className="mt-8 space-y-4">
          ...
          <button onClick={handleOrcidLogin} className="w-full flex items-center justify-center px-4 py-2 border border-green-600 rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
          ...
          <form className="space-y-4" onSubmit={handleMagicLink}>
            ...
            />
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50">
              {loading ? "Enviando..." : <><Mail className="h-5 w-5 mr-2" /> Entrar com Link Mágico</>}
            </button>
          </form>
        </div>
      </div>
